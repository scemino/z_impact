...
const Entity = struct {
    ...
    on_ground: bool, // True for engine.gravity > 0 and standing on something */ \
    draw_order: i32, // Entities are sorted (ascending) by this before drawing */ \
    physics: EntityPhysics, // Physics behavior */ \
    group: EntityGroup, // The groups this entity belongs to */ \
    check_against: EntityGroup, // The groups that this entity can touch */ \
    pos: Vec2, // Top left position of the bounding box in the game world; usually not manipulated directly */ \
    size: Vec2, // The bounding box for physics */ \
    vel: Vec2, // Velocity */ \
    accel: Vec2, // Acceleration */ \
    friction: Vec2, // Friction as a factor of engine.tick * velocity */ \
    offset: Vec2, // Offset from position to draw the anim */ \
    name: []const u8, // Name used for targets etc. usually set through json data */ \
    health: f32, // When entity is damaged an resulting health < 0, the entity is killed */ \
    gravity: f32, // Gravity factor with engine.gravity. Default 1.0 */ \
    mass: f32, // Mass factor for active collisions. Default 1.0 */\
    restitution: f32, // The "bounciness factor" */ \
    max_ground_normal: f32, // For slopes, determines on how steep the slope can be to set on_ground flag. Default cosf(to_radians(46)) */ \
    min_slide_normal: f32, // For slopes, determines how steep the slope has to be for entity to slide down. Default cosf(to_radians(0)) */ \
    // TODO: anim: anim_t , // The animation that is automatically drawn */ \
};
...
const EntityVtab = struct {
    ...
    // of your entity.
    init: ?*const fn (self: *Entity) void = null,
    ...
    damage: ?*const fn (self: *Entity, other: *Entity, damage: f32) void = null,

    // Called through entity_trigger()
    trigger: ?*const fn (self: *Entity, other: *Entity) void = null,

    // Called through entity_message()
    // TODO: message: ?*const fn (self: *Entity, message: EntityMessage, data: anytype) void = null,
};
